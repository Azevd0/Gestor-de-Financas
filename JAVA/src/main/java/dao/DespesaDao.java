...
public class DespesaDao {
	...
	public Despesa buscarPorId(Long id) {
		...
		try {
			return emf.find(Despesa.class, id);
		} finally {
			emf.close();
		}
	}
	...
	public List<Despesa> buscarTodas() {
		...
		try {
			return emf.createQuery("SELECT d FROM Despesa d", Despesa.class).getResultList();
		} finally {
			emf.close();
		}
	}
	...
	public List<Despesa> buscarPorMesAno(int mes, int ano) {
		...
		try {
			return emf.createQuery(
					"SELECT d FROM Despesa d WHERE EXTRACT(MONTH FROM d.dataCadastro) = :mes AND EXTRACT(YEAR FROM d.dataCadastro) = :ano",
					Despesa.class).setParameter("mes", mes).setParameter("ano", ano).getResultList();
		} finally {
			emf.close();
		}
	}
	...
	public int excluirMesAno(int mes, int ano) {
		...
		try {
			...
			int totalDeletado = em.createQuery(
					"DELETE FROM Despesa d WHERE EXTRACT(MONTH FROM d.dataCadastro) = :mes AND EXTRACT(YEAR FROM d.dataCadastro) = :ano")
					.setParameter("mes", mes).setParameter("ano", ano).executeUpdate();
